...
from .constants import APP_TIP, ICON_PATH
...
class TrayIcon:
    ...
    参数均为回调, 且在非主线程中被调用, 回调内部必须自行投递到主线程:
        on_show()    显示主窗口
        on_pause()   暂停/恢复翻译热键 (传入新的暂停状态)
        on_exit()    退出程序
    """

    def __init__(self, on_show, on_pause, on_exit):
    ...
    def _build_menu(self):
        return pystray.Menu(
            pystray.MenuItem("打开主窗口", self._safe(self._show), default=True),
            pystray.MenuItem(
                "暂停翻译热键",
